...
gmn_add <- gmn_add %>%
  ...
  rbind(data.frame(
    unit_id = sample$unit_id[1],
    osm_id = c(
      824561810, 627622410, 1119616340,
...
gmn_remove <- gmn_remove %>%
  rbind(data.frame(
    unit_id = sample$unit_id[1],
    osm_id = c(
      627622404, 1104990471, 954710545,
...
gmn_add <- gmn_add %>%
  rbind(
    data.frame(
      ...
      osm_id = c(
        892603678, 992972329, 1094970384,
        1094970383, 18199515
      )
    )
  )
...
gmn_new_to_pull <- gmn_new_to_pull %>%
  rbind(data.frame(
    unit_id = "Great-Northern_trail_Greater-MN",
    osm_id = c(
      138770612, 1027046763, 1027046764,
...
gmn_remove <- gmn_remove %>%
  rbind(
    data.frame(
      unit_id = sample$unit_id[1],
      osm_id = c(
        1141856624, 1141547206, 18233799,
        18252319, 214340509
      )
    )
  )
