...
if(initiate_coverage_review == TRUE){
  run_trail_21_analysis(.analysis_name = paste0("dnr_", analysis_date), 
                        .segment_zone_set = dnr_segment_zones, 
                        .buffer_zone_set = dnr_buffer_zones)
  Sys.sleep(6)
  run_trail_21_analysis(.analysis_name = paste0("gmn_", analysis_date), 
                        .segment_zone_set = gmn_segment_zones, 
                        .buffer_zone_set = gmn_buffer_zones)
  Sys.sleep(6)
  run_trail_21_analysis(.analysis_name = paste0("metro_", analysis_date), 
...
fetch_trail_coverage_data <- function(.analysis_name){
  ...
  summer21_demos <- bind_rows(full_join(bike_summer_equity, bike_summer_education_income), 
                              full_join(ped_summer_equity, ped_summer_education_income))
  full21_demos <- bind_rows(full_join(bike_full_equity, bike_full_education_income), 
