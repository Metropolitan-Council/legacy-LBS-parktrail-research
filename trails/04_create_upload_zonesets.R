...
new_dnr_segments_geog <- purrr::map_dfr(.x = c(1:nrow(dnr_new_to_pull)), .f = function(x){
  pull <- opq_osm_id(id = dnr_new_to_pull[x, ]$osm_id, 
                     type = "way", 
                     open_url = FALSE) %>%
    osmdata_sf()
  ...
  lines <- pull$osm_lines %>%
    mutate(unit_id = dnr_new_to_pull[x, ]$unit_id, 
...
new_gmn_segments_geog <- purrr::map_dfr(.x = c(1:nrow(gmn_new_to_pull)), .f = function(x){
  pull <- opq_osm_id(id = gmn_new_to_pull[x, ]$osm_id, 
                     type = "way", 
                     open_url = FALSE) %>%
    osmdata_sf()
  ...
  if(str_detect(gmn_new_to_pull[x,]$unit_id, "Cook-County-Mountain-Bike")){
    pull$osm_polygons %>%
      st_cast("LINESTRING") %>%
      mutate(unit_id = gmn_new_to_pull[x, ]$unit_id, 
             osm_id = as.character(osm_id)) %>%
      select(osm_id, unit_id, geometry)
  }
  else{
    pull$osm_lines %>%
      mutate(osm_id = gmn_new_to_pull[x, ]$osm_id, 
