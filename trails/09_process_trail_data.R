...
monthly_segment_volume <- monthly_trails %>%
  ...
  rename(bike = `Bicycle - StL Bicycle Volume`, 
  ...
  mutate(bike_use = bike * days_in_month, 
  ...
  summarise(counter_estimate = sum(bike_use, ped_use), 
  ...
  complete(start_date, day_type, zone_name, 
           fill = list(counter_estimate = 0, bike_counter_estimate = 0, ped_counter_estimate = 0, 
                       miles_traveled = 0, bike_miles_traveled = 0, ped_miles_traveled = 0)) %>%
  # calculate mode share
  mutate(modeshare_bike_miles = bike_miles_traveled/miles_traveled, 
         modeshare_ped_miles = ped_miles_traveled/miles_traveled) %>%
  # replace NaNs with NAs
  mutate(modeshare_bike_miles = ifelse(is.nan(modeshare_bike_miles), 0, modeshare_bike_miles),
         modeshare_ped_miles = ifelse(is.nan(modeshare_ped_miles), 0, modeshare_ped_miles)) %>%
  # join all metadata back in
  left_join(trail_segment_metadata) %>%
  # get month/year
  mutate(month = month(mdy(start_date), label = TRUE), 
         year = year(mdy(start_date)))

correct_trail_meta_test <- filter(monthly_segment_volume, is.na(system))
if(nrow(correct_trail_meta_test) > 0){stop("Some cleaned trail segments are missing correct metadata")}
...
monthly_trail_volume <- monthly_segment_volume %>%
  ...
  summarise(counter_estimate = max(counter_estimate), 
  ...
  mutate(modeshare_bike_miles = bike_miles_traveled/miles_traveled, 
         modeshare_ped_miles = ped_miles_traveled/miles_traveled) %>%
  mutate(modeshare_bike_miles = ifelse(is.nan(modeshare_bike_miles), NA, modeshare_bike_miles),
...
annual_trail_volume <- monthly_trail_volume %>%
  ...
  summarise(counter_estimate = sum(counter_estimate), 
...
hourly <- bind_rows(dnr_hourly$bike_hourly, dnr_hourly$ped_hourly, 
...
hourly_trails <- hourly %>%
  ...
  summarise(sumbike = sum(bike), 
            sumped = sum(ped), 
            sumtotal = sumbike + sumped) %>%
  ungroup() %>%
  group_by(unit_id, day_type) %>%
  mutate(percent_bike = sumbike/sum(sumbike), 
            percent_ped = sumped/sum(sumped), 
            percent_total = sumtotal/sum(sumtotal)) %>%
  mutate(percent_bike = ifelse(is.nan(percent_bike), 0, percent_bike),
