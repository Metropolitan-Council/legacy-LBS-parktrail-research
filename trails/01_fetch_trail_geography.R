...
dnr_meta1 <- dnr_trails %>%
  st_drop_geometry() %>%
  select(
    unit_name, unit_type, unit_type_label, system, system_label,
...
gmn_trails <- gmn %>%
  ...
  mutate(
    unit_name = ifelse(is.na(section),
      # no section
      overall_trail_name,
      # section
...
gmn_meta1 <- gmn_trails %>%
  st_drop_geometry() %>%
  select(
    unit_name, section, unit_type, unit_type_label, system,
...
metro_meta1 <- metro_trails %>%
  st_drop_geometry() %>%
  select(
    short_agency, long_agency, unit_name, section, unit_type, unit_type_label,
