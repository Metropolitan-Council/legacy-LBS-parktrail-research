...
mndot_fix <- mndot %>%
  ...
  mutate(
    latitude = ifelse(str_detect(site, "Luce"), 44.891029, latitude),
...
stl_all <- mndot_trails_monthly_za %>%
  ...
  mutate(
    visitors = as.numeric(average_daily_zone_traffic_st_l_volume),
...
mndot_all <- mndot %>%
  ...
  select(
    date_day, site, date_day, weekend, total, mode,
...
compare_all <- mndot_all %>%
  ...
  filter(
    !zone_name %in% stl_remove$zone_name,
