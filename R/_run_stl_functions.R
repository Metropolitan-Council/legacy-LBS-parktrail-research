    ...
    purrr::map(.x = c(1:nrow(date_sequence)), .f = function(analysis_period) {
      ...
      possibly(
        create_streetlight_analysis(
          ...
          date_ranges = list(
            start_date = date_sequence[analysis_period, ]$start_date,
...
run_home_locations <- function(run = TRUE,
                               fetch = FALSE,
    ...
    purrr::map(.x = c(1:nrow(date_sequence)), .f = function(analysis_period) {
      ...
      possibly(
        create_streetlight_analysis(
          ...
          analysis_type = "Zone_Activity_Analysis",
          analysis_name = paste0(
            analysis_prefix, "_poly_demos_veh_",
            date_sequence[analysis_period, eval(date_sequence_label)]
          ),
          travel_mode_type = "All_Vehicles",
          output_type = "Volume",
      ...
      possibly(
        create_streetlight_analysis(
          ...
          analysis_type = "Zone_Activity_Analysis",
          analysis_name = paste0(
            analysis_prefix, "_poly_demos_bike_",
            date_sequence[analysis_period, eval(date_sequence_label)]
          ),
          travel_mode_type = "Bicycle",
          output_type = "Volume",
      ...
      possibly(
        create_streetlight_analysis(
          ...
          analysis_type = "Zone_Activity_Analysis",
          analysis_name = paste0(
            analysis_prefix, "_poly_demos_ped_",
            date_sequence[analysis_period, eval(date_sequence_label)]
          ),
          travel_mode_type = "Pedestrian",
          output_type = "Volume",
...
run_hourly_activity <- function(
  ...
  if (fetch == TRUE) {
    ...
    bike <- try(get_analysis_data(
      analysis_name = paste0(analysis_prefix, "_hr_bike_", label),
      metric = "za_bike"
    ))
    Sys.sleep(6)
    ped <- try(get_analysis_data(
      analysis_name = paste0(analysis_prefix, "_hr_ped_", label),
      metric = "za_ped"
    ))
    if (trail == FALSE) {
