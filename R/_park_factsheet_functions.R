annual_bar_chart_fxn <- function(x){
  x %>%
    ...
    mutate(annual_active = annual_bike + annual_ped,
           save_active = annual_active,
           save_vehicle = annual_vehicle_visitors,
           save_total = annual_total) %>%
    rename(`Bicycle and pedestrian` = annual_active,
           `Vehicle` = annual_vehicle_visitors) %>%
    pivot_longer(cols = c(`Bicycle and pedestrian`, `Vehicle`),
                 names_to = "Mode",
                 values_to = "Visits") %>%
    ...
    scale_fill_manual(values = c("Bicycle and pedestrian" = legacy_green, 
                                 "Vehicle" = legacy_blue)) + 
    guides(guide = guide_legend(reverse = TRUE)) +
    theme(legend.position = "bottom") +
    
    geom_label(aes(label = prettyNum(save_vehicle, big.mark = ","),
                   y = save_vehicle * 0.5), fill = "white", color = "grey40", size = 3) +
    geom_label(aes(label = prettyNum(save_active, big.mark = ","),
                   y = save_vehicle + save_active * 0.5), fill = "white", color = "grey20", size = 3) +
    geom_label(aes(label = prettyNum(save_total, big.mark = ","),
...
location_map_fxn <- function(name){
  ...
  location_map_text <- paste(meta$unit_label, "is part of the", str_remove(meta$system_label, " \\(Inclusive Boundaries\\)"), "Park system. The park is in",
  ...
  location_map <- leaflet::leaflet() %>%
    ...
    leaflet::addPolygons(data = park_location,
                         fillOpacity = 1,
                         opacity = 1,
                         weight = 0,
...
seasonal_mode_share_fxn <- function(x){
  ...
  x %>%
    ...
    labs(x = "", fill = "Mode", y = "",  title = "Mode share",
    ...
    scale_color_manual(values = c("Vehicle" = legacy_blue,
...
demo_table_fxn <- function(x){
  ...
  demo_tab <- demo_tab %>%
    ...
    regulartable() %>%
    set_header_labels(values = list(category = "Category", group = "Census Group", #summer_park_pct = "Park Estimate (summer 2021)",
...
home_map_fxn <- function(name){
  ...
  home_text <- paste("Inferred visitor home locations are reported at the block group level. Darker colors indicate more visitors from a given block group. During 2021, approximately",
  ...
  home_map <- home %>%
    ...
    geom_sf(data = st_centroid(park_location),
            fill = "#bf812d",
            color = "black",
            shape = 21, size = 2) +
    theme_void() +
