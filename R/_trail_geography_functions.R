fxn_map <- function(.sample_trail){
  ...
  if(map_issues == TRUE){
    mapview::mapview(list(
      .sample_trail,
      filter(trail_polygons, unit_id == .sample_trail$unit_id[1])), col.regions = c("green"),
...
fxn_map_add <- function(.sample_trail, .sysbikeped){
  ...
  if(map_issues == TRUE){
    ...
    leaflet() %>%
      ...
      addPolygons(data = filter(st_transform(trail_polygons, 4326), str_detect(unit_id, .sample_trail$unit_id[1])), 
                  weight = 10, color = "orange", fill = "orange") %>%
      addPolylines(data = idd_segments, 
                   color = "black", weight = 6, 
                   label = ~all_segments$osm_id, 
                   popup = paste0(all_segments$osm_id, "<br>highway: ", all_segments$highway)) %>%
      addPolylines(data = all_segments, 
                   label = ~all_segments$osm_id, 
                   popup = paste0(all_segments$osm_id, "<br>highway: ", all_segments$highway))
  }
}
...
fxn_map_edits <- function(.sample_trail, .added, .removed){
  ...
  if(map_issues == TRUE){
    ...
    if(nrow(added_sf) > 0 & nrow(removed_from_trail) > 0){
      mapview(trail_polygon, col.regions = "green", 
