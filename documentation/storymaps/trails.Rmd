...
cleaned_trail_segments <- cleaned_trail_segments %>%
...
annual_alldays <- annual_trail_volume %>% filter(
...
#               summarise(annual_total = sum(counter_estimate), .groups = "keep")) %>%
#   filter(year == 2020) %>%
...
annual_alldays %>%
  ...
  summarise(miles = sum(miles_traveled) / 1e6, .groups = "keep") %>%
...
```{r trail-screenshot, include = TRUE, eval = TRUE}
