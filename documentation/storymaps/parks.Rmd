...
dnr_monthly_comp <- dnr_monthly %>%
  ...
  summarize(LBS = sum(monthly_total),
            DNR = sum(dnr_count)) %>%
  ungroup() %>%
  group_by(NAME, year) %>%
...
dnr_monthly_comp %>% #filter(year != 2021) %>%
  ...
  labs(x = "DNR use estimates", y = "LBS use \nestimates",
...
system_vis <- annual_volume %>% 
  ...
  theme_council_open(base_size = 10) +
  labs(x = "", caption = fig_caption, #y = "Annual\npark\nvisitation\n(millions)"
...
```{r season-calc}
top_season <- (monthly_volume) %>% filter(year == 2021) %>%
...
annual_volume %>% filter(year != 2022) %>%
