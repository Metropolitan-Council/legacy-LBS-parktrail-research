...
trail_summary <- st_drop_geometry(cleaned_trail_segments) %>%
  ...
  summarise(Trails = n_distinct(unit_id),
            ...
            Miles = round(sum(as.numeric(approx_length_miles)))) 
...
working_annual_volume %>%
  group_by(year) %>%
  summarise(use = sum(annual_total)/1e6) %>%
  mutate(type = "Park use\n(millions of visitors)") %>%
  
  bind_rows(working_trail_volume %>%
              group_by(year) %>%
              summarise(use = sum(miles_traveled)/1e6) %>%
  ...
  # geom_text(aes(label = use), vjust = -.2) +
    geom_label(aes(label = format(round(use, 2), nsmall = 2), y = rep(c(30.5, 49), each = 3)), fill = "white", size = 4.5) +
