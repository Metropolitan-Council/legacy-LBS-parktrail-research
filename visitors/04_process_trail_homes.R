...
zips <- zips1 %>%
  ...
  ungroup() %>%
  group_by(label, day_type, day_part, zone_name) %>%
  mutate(zone_total = sum(total_by_zip), 
...
states <- states1 %>%
  ...
  ungroup() %>%
  group_by(label, day_type, day_part, zone_name) %>%
  mutate(zone_total = sum(total_by_state), 
