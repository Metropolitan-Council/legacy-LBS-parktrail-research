...
##### incorporate survey ######
park_to_zone <- data.frame(zone_name = c("Battle-Creek_park_Metro-Regional_Ramsey-County",
                           ...
                           unit_name = c("Battle Creek", "Cleary Lake",
                           ...
                           PARK = c("Battle Creek Regional Park",
...
oversample_home <- bind_rows(oversample_survey_home %>%
...
oversample_home_long <- oversample_home %>%
  ...
  pivot_longer(cols = c("LBS", "Survey"),
               names_to = "source",
               values_to = "percent")
...
country_bgs <- purrr::map_dfr(.x = c(1:n_distinct(home_states$state_name)), .f = function(x){
  tigris::block_groups(state = unique(home_states$state_name)[x],
                       year = 2020, cb = TRUE)
})

park_bgs <- home_bgs %>%
  filter(unit_type == "park", 
         str_detect(day_type, "All"), 
         str_detect(day_part, "All"), 
         label %in% c("year21", "full21")) %>%
  arrange(system, district, unit_id) %>%
  select(shp_id, block_group_id, percent_by_bg) %>%
  transmute(shp_id = shp_id, 
