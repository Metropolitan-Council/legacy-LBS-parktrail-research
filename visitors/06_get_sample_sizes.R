...
## oversampled units
oversampled_units <- 
  c("Battle-Creek_park_Metro-Regional_Ramsey-County",
...
purrr::map(.x = 1:nrow(geos), .f = function(x){
  ...
  if(str_detect(zone$zone_name, "_park_")){
    try(create_streetlight_analysis(login_email = login_email, 
                                ...
                                analysis_name = paste0(substr(paste0(zone$zone_name), 1, 20), 
                                                       "_veh_ss"), 
                                date_ranges = list(start_date = "06/01/2021", 
                                                   end_date = "08/31/2021"), 
...
sample_size_raw <- purrr::map_dfr(.x = 1:nrow(geos), .f = function(x){
  ...
  Sys.sleep(6)
  bike <- get_analysis_data(analysis_name =  paste0(substr(paste0(zone$zone_name), 1, 20), "_bike_ss"), 
                            metric = "sample_size") %>%
    mutate_all(as.character)
  
  Sys.sleep(6)
  ped <- get_analysis_data(analysis_name =  paste0(substr(paste0(zone$zone_name), 1, 20), "_ped_ss"), 
                           metric = "sample_size") %>%
    mutate_all(as.character)
  
  if(str_detect(zone$zone_name, "_park_")){
    Sys.sleep(6)
    veh <- get_analysis_data(analysis_name =  paste0(substr(paste0(zone$zone_name), 1, 20), "_veh_ss"), 
                             metric = "sample_size") %>%
      mutate_all(as.character)
