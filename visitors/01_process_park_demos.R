...
equity1 <- bind_rows(dnr_home_locations$equity,
                     gmn_home_locations$equity, 
                     metro_home_locations$equity)

edu_inc1 <- bind_rows(dnr_home_locations$education_income,
...
veh_equity <- equity1 %>%
  ...
  select(-c(zone_id, data_periods, home_and_work_filter, intersection_type, zone_is_pass_through, 
  ...
  mutate_at(vars(-one_of(c("mode_of_travel", "unit_id", "zone_name", "day_part", 
                           "day_type", "label"))), as.numeric) %>%
  rowwise() %>%
  # get new denominator
  ...
  mutate_at(vars(c(white, black, american_indian, asian, pacific_islander, 
                   other_race, multiple_races, hispanic)), ~./totalp) %>%
  ...
  mutate_at(vars(-one_of(c("mode_of_travel", "zone_name", "day_type", "day_part", 
                           "label", "unit_id", "vehicle_multiplier", "vehicle_visitors"))),  ~.*vehicle_visitors)
...
equity <- bind_rows(veh_equity, bike_equity, ped_equity) %>%
  ...
  mutate_at(vars(-one_of("zone_name", "unit_id", "label", "day_type", 
                         "day_part", "total_volume")), ~./total_volume) %>%
