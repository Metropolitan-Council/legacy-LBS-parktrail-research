...
states <- tigris::states(cb = TRUE) %>%
  filter(!NAME %in% c(
    ...
    "Hawaii", "United States Virgin Islands",
...
full_ss_comp %>%
  ...
  add_header_row(
    values = c("", "Intercept Survey", "LBS Data"),
    colwidths = c(1, 2, 2)
  ) %>%
...
all_cordat %>%
  ...
  add_header_row(
    values = c("", "Zip codes identified", "Correlation"),
    colwidths = c(1, 3, 2)
  ) %>%
  align(align = "center", i = 1, part = "header") %>%
  bold(bold = TRUE, part = "header") %>%
...
mn <- tigris::states(cb = TRUE, progress_bar = FALSE) %>% filter(NAME == "Minnesota")
...
both %>%
  ...
  mutate(datlabel = paste0(
    scales::percent(percent, accuracy = 0.1),
    " (±", scales::percent(moe_95, accuracy = 0.01), ")"
  )) %>%
