...
```{r run-parks}
## set run_ parameters to TRUE to run StreetLight analyses, set fetch_ parameters TRUE to fetch data
...
monthly_volume %>%
  ...
  mutate(
    label = ifelse(month == 12 & str_detect(
      unit_label,
      ...
      levels = c(
        ...
        unique(
          filter(
            ...
            !str_detect(
              unit_label,
              "Spirit Mountain|Detroit Mountain|Anderson Lakes, Three Rivers"
            )
          )$unit_label
        )
      ),
