...
both_monthly2 %>%
  ...
  summarise(
    `Count-based estimate` = mean(dnr_count, na.rm = TRUE), SE_DNR = sd(dnr_count, na.rm = TRUE) / sqrt(n()),
...
monthly_volume %>%
  ...
  mutate(
    label = ifelse(month == 12 & str_detect(unit_label, "Spirit Mountain|Detroit Mountain"), unit_label, NA_character_),
    ...
    unit_label = factor(unit_label,
      levels = c(
        "Spirit Mountain", "Detroit Mountain", "Hyland-Bush-Anderson Lakes, Three Rivers",
