...
dnr <- st_read(paste0(here(), "/data-intermediate/parks/shp/dnr_", 
                      park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 

dnr_perim <- st_read(paste0(here(), "/data-intermediate/parks/shp/dnr_perim_", 
                            park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 

gmn <- st_read(paste0(here(), "/data-intermediate/parks/shp/gmn_", 
                      park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 

gmn_perim <- st_read(paste0(here(), "/data-intermediate/parks/shp/gmn_perim_", 
                            park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 

metro <- st_read(paste0(here(), "/data-intermediate/parks/shp/metro_", 
                      park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 

metro_perim <- st_read(paste0(here(), "/data-intermediate/parks/shp/metro_perim_", 
                            park_geom_processing_date, ".shp")) %>%
  select(name = zone_name, geometry) 
...
st_write(zones_meta, paste0(here(), "/data-processed/zone-sets/park-polygons/park-polygon-zone-set-", 
                       park_geom_processing_date, ".shp"), append = FALSE)
...
st_write(perim_zones_meta, paste0(here(), "/data-processed/zone-sets/park-perimeters/park-perim-zone-set-", 
                       park_geom_processing_date, ".shp"), append = FALSE)
