...
set.seed(1997)
random_sample <- sample(n_weeks, 30)
...
no_missing <- week_vehicle %>%
  filter(
    zone_name %in% no_missing_names$zone_name,
    str_detect(day_type, "All"),
    str_detect(day_part, "All"),
    intersection_type == "Trip End"
  ) %>%
...
manual_missing <- manual_missing1 %>%
  group_by(zone_name) %>%
  mutate(
    imputed_volume = na_seadec(ts(missing_volume, frequency = 52), algorithm = "ma"),
...
eval_dat <- purrr:::map_dfr(.x = c(1:500), .f = function(rep) {
  man_missing <- no_missing %>%
    group_by(zone_name) %>%
    mutate(
      random_index = list(sample(n_weeks, 30)),
      row_number = row_number()
    ) %>%
    mutate(
      missing_volume = ifelse(row_number %in% unlist(random_index), NA, weekly_vehicles),
  ...
  eval_dat <- man_missing %>%
    mutate(
      linear_resid = linear_volume - weekly_vehicles,
      spline_resid = spline_volume - weekly_vehicles,
      ma_resid = ma_volume - weekly_vehicles
    ) %>%
    mutate(
      linear_rel = 100 * linear_resid / weekly_vehicles,
...
eval <- eval_dat %>%
  tidyr::pivot_longer(
    cols = c("linear_volume", "spline_volume", "ma_volume"),
    names_to = "method"
  ) %>%
  ...
  mutate(
    min_MAPE = min(MAPE_linear_volume, MAPE_ma_volume, MAPE_spline_volume),
    min_RMSE = min(RMSE_linear_volume, RMSE_ma_volume, RMSE_spline_volume),
    best_MAPE = case_when(
      min_MAPE == MAPE_linear_volume ~ "linear",
      min_MAPE == MAPE_ma_volume ~ "ma",
      min_MAPE == MAPE_spline_volume ~ "spline"
    ),
    best_RMSE = case_when(
      min_RMSE == RMSE_linear_volume ~ "linear",
