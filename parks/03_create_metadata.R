...
inclusive_metadata <- basic_metadata1 %>%
  ...
  mutate(zone_name = str_replace_all(zone_name, "Metro-Regional", "Metro-Regional-Inclusive"), 
...
dnr_area_check <- dnr_area_county %>%
  group_by(county) %>% summarise(count = n()) %>% filter(count > 1)
