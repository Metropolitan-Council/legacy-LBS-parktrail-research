...
mndot_rm <- roads %>%
  filter(!ROUTE_NAME %in% c("Swift T 363-D", "Swift T 363-I", 
...
park %>% 
  ...
  select(zone_name, geometry) %>% saveRDS(paste0(here(), "/data-temp/greatermn-park-geography/", str_replace_all(park$zone_name, " ", "_"), ".RDS"))
```
...
# okayish
mndot_rm <- roads %>%
  filter(!ROUTE_NAME %in% c("Glenwood M 120-D", "Glenwood M 120-I",
...
park %>% 
  st_make_valid() %>%
  st_erase(mndot_rm) %>%  #mapview()
  select(zone_name, geometry) %>% saveRDS(paste0(here(), "/data-temp/greatermn-park-geography/", str_replace_all(str_replace_all(park$zone_name, " ", "_"), "/", "_"), ".RDS"))
```
...
mndot_rm <- roads %>%
  filter(!ROUTE_NAME %in% c("Grand Marais M 36-I", "Grand Marais M 36-D", 
...
list_of_files <- as.list(paste0(here(), "/data-temp/greatermn-park-geography/", 
...
st_write(gmn_parks, paste0(here(), "/data-intermediate/partial-park-zones/gmn_", 
...
```{r}
gmn_parks <- read_sf(paste0(here(), "/data-intermediate/partial-park-zones/gmn_", 
