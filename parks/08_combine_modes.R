...
veh_expanded <- vehicle[, list(
  zone_name = zone_name, mode = mode, year = year, start_date = start_date, end_date = end_date, label = label,
...
monthly_vehicle <- veh_expanded %>%
  mutate(
    ...
    month = month(date),
    vehicle_source = factor(vehicle_source,
      levels = c("Weekly StreetLight", "Biweekly StreetLight", "Monthly StreetLight", "Imputed"),
      ordered = TRUE
    )
  ) %>%
...
all_modes_expanded <- veh_expanded %>%
  ...
  rowwise() %>%
  mutate(
    bike = max(ceiling(vehicles * bike_scaling_factor), 0),
...
all_modes_expanded <- all_modes_expanded %>%
  ...
  mutate(
    vehicle_visitors = ceiling(vehicles * vehicle_multiplier),
...
hourly_parks <- do.call(bind_rows, hourly) %>%
  ...
  rename(
    vehicles = `All Vehicles LBS Plus - StL All Vehicles Volume`,
