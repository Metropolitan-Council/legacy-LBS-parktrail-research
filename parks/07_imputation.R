...
start_dates <- unique(filled_weekly_results$start_date) %>% as.Date() %>% sort()
end_dates <- unique(filled_weekly_results$end_date) %>% as.Date() %>% sort()
...
for(i in 1:length(needs_vehicle_imputation)){
  ...
  # save results
  new_results <- data.frame(zone_name = needs_vehicle_imputation[i], 
                            volume = vol, 
...
veh_results <- veh_results %>%
  mutate(imputed = ifelse((is.na(volume) & !is.na(imputed_volume)),
                          TRUE, FALSE),
...
weekly <- filled_weekly_results %>%
  ...
  mutate(source = ifelse(is.na(source), "Imputed", source),
...
summarytab <- weekly %>%
  group_by(source) %>%
  summarise(count = n()) %>% as.data.table()
...
bike_monthly1 <- month_bike %>%
  ...
  mutate(raw_bike = as.numeric(average_daily_zone_traffic_st_l_volume),
...
bike_monthly <- bike_monthly1 %>%
  ...
  rowwise() %>%
  mutate(od_removed = ifelse(!is.na(daily_bike_intrapark_trips), TRUE, FALSE),
...
bike_monthly_expanded <- bike_monthly %>%
  ...
  bind_rows(no_bike_data %>% 
              mutate(start_date = as.Date("2019-01-01"), 
...
no_bike_imputation <- bike_monthly_expanded %>%
  group_by(zone_name) %>%
  summarise(missing = sum(is.na(bike_volume)), 
            count = n() , 
...
needs_bike_imputation <- bike_monthly_missing %>%
  ...
  select(zone_name) %>% unique()

month_start_dates <- unique(bike_monthly_expanded$start_date) %>% as.Date() %>% sort()
month_end_dates <- unique(bike_monthly_expanded$end_date) %>% as.Date() %>% sort()
...
for(i in 1:nrow(needs_bike_imputation)){
  ...
  # save results
  new_results <- data.frame(zone_name = needs_bike_imputation[i,]$zone_name, 
                            bike_volume = vol, 
...
ped_monthly1 <- month_ped %>%
  ...
  filter(str_detect(day_part, "All"), 
         str_detect(day_type, "All"), 
         intersection_type == "Trip End") %>%
  mutate(raw_ped = as.numeric(average_daily_zone_traffic_st_l_volume),
         source = "Monthly StreetLight",
         start_date = as.Date(start_date, format = "%m/%d/%Y"),
         end_date = as.Date(end_date, format = "%m/%d/%Y")) %>%
  select(zone_name, start_date, end_date, label, month, year,
...
ped_monthly <- ped_monthly1 %>%
  ...
  rowwise() %>%
  mutate(od_removed = ifelse(!is.na(daily_ped_intrapark_trips), TRUE, FALSE),
...
no_ped_imputation <- ped_monthly_expanded %>%
  group_by(zone_name) %>%
  summarise(missing = sum(is.na(ped_volume)), 
            count = n() , 
...
for(i in 1:nrow(needs_ped_imputation)){
  ...
  # save results
  new_results <- data.frame(zone_name = needs_ped_imputation[i,]$zone_name, 
                            ped_volume = vol, 
