...
data(fips_codes)
state_abbr <- fips_codes %>% select(state_code, state_name) %>% unique() %>%
  as_tibble()
...
create_unit_fig <- function(name){
  ...
  explain_text <- paste("This factsheet summarizes park-level data for",  meta$unit_label, paste0(meta$unit_type_label, "."),
                        ...
                        "\n\nThis research project was funded with Legacy Partnership Research Funds from the State of Minnesota Parks and Trails Legacy Fund. LBS data was obtained from StreetLight Data, Inc. and was accessed in July 2023.") # More information is available on the <a href = 'https://minnesota-parks-and-trails-metrocouncil.hub.arcgis.com/'> interactive project website</a>.
  ...
  explainer <- textbox_grob(explain_text, use_markdown = T,
  ...
  top <- plot_grid(title,
                   plot_grid(
                     ...
                     loc_map,
                     nullGrob(), nrow = 1, rel_widths = c(0.5, 5, 3, 0.2)
                   ),
                   ncol = 1, rel_heights = c(1, 5))
  
  middle <- plot_grid(nullGrob(),
                      season_mode, hour_ts,
                      nullGrob(), nrow = 1, rel_widths = c(0.05, 1, 1, 0.1))
  ...
  if (meta$system == "Metro Regional") {
    save_plot(figure, filename = paste0(here::here(), "/figures/factsheets/", meta$system, "-parks/", stringr::str_replace(meta$unit_label, "/", "-"), "_", meta$short_agency, ".pdf"),
              base_height = 23, base_width = 17)
  } else {
    save_plot(figure, filename = paste0(here::here(), "/figures/factsheets/", meta$system, "-parks/", stringr::str_replace(meta$unit_label, "/", "-"), ".pdf"),
              base_height = 23, base_width = 17)
