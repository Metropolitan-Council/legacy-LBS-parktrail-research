...
weekly_results1 <- week_vehicle %>%
  ...
  mutate(
    raw_volume = as.numeric(average_daily_zone_traffic_st_l_volume), # raw_volume = numbers from StL, no edits
    source = "Weekly StreetLight",
    start_date = as.Date(start_date, format = "%m/%d/%Y"),
    end_date = as.Date(end_date, format = "%m/%d/%Y")
  ) %>%
  select(
    zone_name, start_date, end_date, label, week_of_year, year,
...
weekly_results2 <- weekly_results1 %>%
  ...
  mutate(
    od_removed = ifelse(!is.na(daily_intrapark_trips), TRUE, FALSE),
    removed_daily_intrapark_trips = replace_na(daily_intrapark_trips, 0),
    volume = raw_volume - removed_daily_intrapark_trips
  ) # remove intrapark trips right away
...
fillin_monthly <- still_missing_monthly %>%
  ...
  mutate(
    total_week_volume = days_in_month1 * month1_volume + days_in_month2 * month2_volume,
    monthly_volume = round(total_week_volume / 7)
  ) %>% # get "average daily" to match other sources
  ungroup() %>%
...
filled_weekly_results <- plyr::rbind.fill(filled_weekly_results1, fillin_monthly) %>%
  ...
  select(
    zone_name, start_date, end_date, label, week_of_year, year, source,
  ...
  unique() %>%
  mutate(
    source = ifelse(is.na(volume), NA, source),
