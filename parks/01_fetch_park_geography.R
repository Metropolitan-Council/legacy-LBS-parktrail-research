...
dnr_management_boundaries <- read_sf(unzip(temp_dnr, "bdry_dnr_lrs_prk.gpkg"), layer = "dnr_management_units_prk") %>%
  ...
  mutate(
    extra_id = ifelse(UNIT_TYPE == "State Park", NA, str_replace_all(UNIT_TYPE, " ", "-")),
...
mc_raw <- sf::read_sf(unzip(temp_mc, "plan_parks_regional.gpkg")) %>%
  mutate(
    AGENCY = ifelse(AGENCY == "St. Paul Parks and Recreation",
...
tierneys_woods <- c(
  "053-1911621220001", "053-1811621330001", "053-1811621340001", "053-1911621210042",
...
hyland_unit <- c(
  "053-1711621430012", "053-1711621430012", "053-2911621210002", "053-2911621320003",
...
metro <- mc_raw %>%
  ...
  mutate(
    zone_name = str_replace_all(paste0(NAME, "_park_Metro-Regional_", short_agency), " ", "-"),
    zone_name = str_remove_all(zone_name, "\\."),
    zone_name = str_replace_all(zone_name, "\\/", "-"),
